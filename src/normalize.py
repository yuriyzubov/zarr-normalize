...
def renormalize_and_save_slice(
    ...
    filtered_data = np.where((source_data >= global_min) & (source_data <= global_max), (source_data-global_min) * (np.iinfo(dest_dtype).max / (global_max - global_min)), 0)
...
def copy_arrays(z_src: zarr.Group | zarr.core.Array,
    ...
    for src_arr in z_arrays:
        ...
        if dst_data_type == '':
            dest_dtype = src_arr.dtype
        else:
            dest_dtype = dst_data_type
        ...
        # increase size of the slice = chunksize * 3, to reduce underutilization of workers
        out_slices = slices_from_chunks(normalize_chunks([_ * 3 for _ in dest_arr.chunks], shape=dest_arr.shape))
        ...
        for idx, part in enumerate(out_slices_partitioned):
            ...
            start = time.time()
            fut = client.map(lambda v: renormalize_and_save_slice(src_arr, dest_arr, v, global_min, global_max, dest_dtype), part)
...
def cli(src,
    ...
    copy_arrays(z_src=source_obj, dest_root=dest_root, global_min=global_min, global_max=global_max, client=client, num_workers=workers, dst_data_type=data_type, comp=Zstd(level=6))
